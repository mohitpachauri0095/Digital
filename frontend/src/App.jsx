...
const Navbar = () => {
  ...
  return (
    <motion.header 
      ...
      className="bg-college-dark/95 backdrop-blur-md text-college-light sticky top-0 z-50 border-b border-college-accent/20 shadow-lg transition-colors duration-300"
    >
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <Link to="/">
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
            className="text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-college-light to-college-accent transition-transform"
          >
            MVD University
          </motion.div>
        </Link>
        <div className="flex items-center gap-8">
          <nav className="hidden md:flex gap-6">
            {navLinks.map((link, index) => (
              ...
              >
                <Link 
                  to={link.path} 
                  className={`font-semibold tracking-wide transition-all pb-1 border-b-2 hover:-translate-y-0.5 block ${
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.9 }}
            onClick={toggleTheme} 
            className="p-2.5 rounded-xl border border-college-secondary/30 bg-college-primary/50 shadow-sm transition-colors"
